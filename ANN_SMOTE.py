...
logging.basicConfig(filename="logANN_SMOTE",
                    filemode='a',
                    format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                    datefmt='%H:%M:%S',
                    level=logging.INFO)

logging.info("Running ANN_SMOTE.py")

logger = logging.getLogger('ANN_SMOTE')
...
model.add(layers.Dense(4096, activation='relu', input_dim=2560))
...
model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
    filepath= 'modelANN_SMOTE.{epoch:02d}-{accuracy:.4f}-{val_accuracy:.4f}.h5',
...
filepath = "modelANN_SMOTE-" + "val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
...
filepath = "modelANN_SMOTE-" + "val_loss-" + str(max(history.history['val_loss'])) + ".png"
...
filepath = "modelANN_SMOTE." + inttostring(history.history['val_accuracy'].index(max(history.history['val_accuracy'])) + 1) + "-"
...
filepath = "modelANN_SMOTE_confusion_matrix" + "val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
