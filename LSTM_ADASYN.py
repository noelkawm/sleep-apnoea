...
logging.basicConfig(filename="logLSTM_ADASYN",
                    filemode='a',
                    format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                    datefmt='%H:%M:%S',
                    level=logging.INFO)

logging.info("Running LSTM_ADASYN.py")

logger = logging.getLogger('LSTM_ADASYN')
...
model.add(layers.LSTM(512,input_shape=(2560,1),return_sequences=False))

model.add(layers.Dense(4096, kernel_initializer='normal',activation='linear'))
model.add(layers.Dense(2048, kernel_initializer='normal',activation='linear'))
model.add(layers.Dense(1024, kernel_initializer='normal',activation='linear'))
model.add(layers.Dense(512, kernel_initializer='normal',activation='linear'))
model.add(layers.Dense(256, kernel_initializer='normal',activation='linear'))
model.add(layers.Dense(128, kernel_initializer='normal',activation='linear'))
model.add(layers.Dense(32, kernel_initializer='normal',activation='linear'))
model.add(layers.Dense(16, kernel_initializer='normal',activation='linear'))
...
model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
    filepath= 'modelLSTM_ADASYN.{epoch:02d}-{accuracy:.4f}-{val_accuracy:.4f}.h5',
...
filepath = "modelLSTM_ADASYN-" + "val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
...
filepath = "modelLSTM_ADASYN-" + "val_loss-" + str(max(history.history['val_loss'])) + ".png"
...
filepath = "modelLSTM_ADASYN." + inttostring(history.history['val_accuracy'].index(max(history.history['val_accuracy'])) + 1) + "-"
...
filepath = "modelLSTM_ADASYN_confusion_matrix" + "val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
