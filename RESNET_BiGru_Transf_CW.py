...
def createModel(epochs, batch_size, noRows, GRU_units):
    ...
    logging.basicConfig(filename="logRESNET_BiGru_Transf_CW ",
                        filemode='a',
                        format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                        datefmt='%H:%M:%S',
                        level=logging.INFO)
    
    logging.info("Running RESNET_BiGru_Transf_CW .py")
    ...
    logger = logging.getLogger('RESNET_BiGru_Transf_CW ')
    ...
    callbacks.append(ModelCheckpoint(filepath= 'model_RESNET_BiGru_Transf_CW -' + str(epochs) + "-" + str(batch_size) + '.h5',
    ...
    history = combined_model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=epochs, batch_size=batch_size, callbacks=callbacks, class_weight=class_weight_dict)
    logger.info('Model is trained')
    
    filepath = "modelRESNET_BiGru_Transf_CW -" + str(epochs) + "-" + str(batch_size) + "-val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
    ...
    filepath = "modelRESNET_BiGru_Transf_CW -" + str(epochs) + "-" + str(batch_size) + "-val_loss-" + str(max(history.history['val_loss'])) + ".png"
    ...
    filepath = "modelRESNET_BiGru_Transf_CW ." + inttostring(history.history['val_accuracy'].index(max(history.history['val_accuracy'])) + 1) + "-"
    filepath = 'model_RESNET_BiGru_Transf_CW -' + str(epochs) + "-" + str(batch_size) + '.h5'
    ...
    filepath = "modelRESNET_BiGru_Transf_CW _confusion_matrix-" + str(epochs) + "-" + str(batch_size) + "-val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
...
def main(argv):
    ...
    try:
       ...
       print('RESNET_BiGru_Transf_CW .py -e <no epochs (default 3)> -b <batch size (default 64) > -n <number of rows to load from combined_data.csv> (default 500) -g <Number of GRU units>')
       sys.exit(2)
    for opt, arg in opts:
       if opt == '-h':
          print('RESNET_BiGru_Transf_CW .py -e <no epochs (default 3)> -b <batch size (default 64) > -n <number of rows to load from combined_data.csv> (default 500, None for all rows) -g <Number of GRU units>')
          sys.exit()
       elif opt in ("-e"):
          epochs = int(arg)       
       elif opt in ("-g"):
