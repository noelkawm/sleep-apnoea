...
def createModel(epochs, batch_size, noRows):
    ...
    model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath= 'model_LSTM_SMOTE-' + str(epochs) + "-" + str(batch_size) + '.h5',
    ...
    filepath = "modelLSTM_SMOTE-" + str(epochs) + "-" + str(batch_size) + "-val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
    ...
    filepath = "modelLSTM_SMOTE-" + str(epochs) + "-" + str(batch_size) + "-val_loss-" + str(max(history.history['val_loss'])) + ".png"
    ...
    filepath = "modelLSTM_SMOTE_confusion_matrix-" + str(epochs) + "-" + str(batch_size) + "-val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
...
def main(argv):
    ...
    try:
       ...
       print('LSTM_SMOTE.py -e <no epochs (default 3)> -b <batch size (default 64) > -n <number of rows to load from combined_data.csv> (default 500)')
       sys.exit(2)
    for opt, arg in opts:
       if opt == '-h':
          print('LSTM_SMOTE.py -e <no epochs (default 3)> -b <batch size (default 64) > -n <number of rows to load from combined_data.csv> (default 500, None for all rows)')
          sys.exit()
       elif opt in ("-e"):
          epochs = int(arg)       
       elif opt in ("-b"):
