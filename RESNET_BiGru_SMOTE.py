...
def createModel(epochs, batch_size, noRows):
    ...
    logging.basicConfig(filename="logRESNET_BiGru_SMOTE",
                        filemode='a',
                        format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                        datefmt='%H:%M:%S',
                        level=logging.INFO)
    
    logging.info("Running RESNET_BiGru_SMOTE.py")
    ...
    logger = logging.getLogger('RESNET_BiGru_SMOTE')
    ...
    gru_output = Bidirectional(GRU(64, return_sequences=False))(reshaped_output) 
    ...
    callbacks.append(ModelCheckpoint(filepath= 'model_RESNET_BiGru_SMOTE-' + str(epochs) + "-" + str(batch_size) + '.h5',
    ...
    filepath = "modelRESNET_BiGru_SMOTE-" + str(epochs) + "-" + str(batch_size) + "-val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
    ...
    filepath = "modelRESNET_BiGru_SMOTE-" + str(epochs) + "-" + str(batch_size) + "-val_loss-" + str(max(history.history['val_loss'])) + ".png"
    ...
    filepath = "modelRESNET_BiGru_SMOTE." + inttostring(history.history['val_accuracy'].index(max(history.history['val_accuracy'])) + 1) + "-"
    filepath = 'model_RESNET_BiGru_SMOTE-' + str(epochs) + "-" + str(batch_size) + '.h5'
    ...
    filepath = "modelRESNET_BiGru_SMOTE_confusion_matrix-" + str(epochs) + "-" + str(batch_size) + "-val_accuracy-" + str(max(history.history['val_accuracy'])) + ".png"
...
def main(argv):
    ...
    try:
       ...
       print('RESNET_BiGru_SMOTE.py -e <no epochs (default 3)> -b <batch size (default 64) > -n <number of rows to load from combined_data.csv> (default 500)')
       sys.exit(2)
    for opt, arg in opts:
       if opt == '-h':
          print('RESNET_BiGru_SMOTE.py -e <no epochs (default 3)> -b <batch size (default 64) > -n <number of rows to load from combined_data.csv> (default 500, None for all rows)')
          sys.exit()
       elif opt in ("-e"):
          epochs = int(arg)       
       elif opt in ("-b"):
